describe('ecommerce product purchasing', () => {
    ...
    it('Order success message displayed with billing details and summary', () => {
        cy.get(':nth-child(5) > .MuiCardContent-root > .mt-auto > .MuiButtonBase-root').click()
        cy.get('.MuiPaper-elevation0 > .MuiToolbar-root > .MuiButtonBase-root').click()
        cy.get('.MuiButton-contained').click()
        
        cy.typeFormField('First Name', 'John')
        cy.typeFormField('Last Name', 'Doe')
        cy.typeFormField('Address', '123 Main St')

        cy.get('.MuiButton-contained').click()
        cy.get('.flex-col > .MuiTypography-root').should('have.text', 'Amount to Pay: $60')
        cy.get('.MuiButton-containedSuccess').click()
        cy.get('.MuiTypography-h5').should('contain.text', 'Order Placed Successfully!')
    })

    it('Failure message displayed with option to return home', () => {
        cy.get(':nth-child(1) > .MuiCardContent-root > .mt-auto > .MuiButtonBase-root').click()
        cy.get('.MuiPaper-elevation0 > .MuiToolbar-root > .MuiButtonBase-root').click()
        cy.get('.MuiButton-contained').click()
        ...
        cy.get('.MuiCardContent-root > .flex > .MuiTypography-root').should('contain.text', 'Payment Failed!')
        cy.get('.MuiCardContent-root > .flex > .MuiButtonBase-root').should('exist').and('not.be.disabled')
    })

    it('Flow resets, showing product listing with empty cart', () => {
        cy.get(':nth-child(5) > .MuiCardContent-root > .mt-auto > .MuiButtonBase-root').click()
        cy.get('.MuiPaper-elevation0 > .MuiToolbar-root > .MuiButtonBase-root').click()
        cy.get('.MuiButton-contained').click()
        ...
        cy.get('.MuiButton-containedSuccess').click()
        cy.get('.MuiTypography-h5').should('contain.text', 'Order Placed Successfully!')
        cy.get('.MuiCardContent-root > .flex > .MuiButtonBase-root').click()
        cy.get('.MuiPaper-elevation0 > .MuiToolbar-root > .MuiButtonBase-root').click()
        cy.get('.space-y-4 > .MuiTypography-root').should('contain.text', 'Total: $0')
    })
});